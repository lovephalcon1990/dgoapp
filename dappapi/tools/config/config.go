...
func ConfigSetup(path string) {
	...
	cfgDatabase = viper.Sub("settings.database.db1")
	if cfgDatabase == nil {
		panic("config not found settings.database.db1")
	}
	DBConfig["admin"] = InitDatabase(cfgDatabase)

	cfgMainDB = viper.Sub("settings.database.db2")
	if cfgMainDB == nil {
		panic("config not found settings.database.db2")
	}
	DBConfig["main"] = InitDatabase(cfgMainDB)

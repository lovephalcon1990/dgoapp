...
func (e *DBTables) GetPage(pageSize int, pageIndex int) ([]DBTables, int, error) {
	...
	table := orm.Eloquent["admin"].Select("*").Table("information_schema.tables")
	table = table.Where("TABLE_NAME not in (select table_name from " + config2.DBConfig["admin"].Name + ".sys_tables) ")
	table = table.Where("table_schema= ? ", config2.DBConfig["admin"].Name)
